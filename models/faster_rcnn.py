...
def load_model(num_classes):
    ...
    model.roi_heads.box_predictor = FastRCNNPredictor(in_feautures, num_classes = num_classes+1)
    return model
...
#시스템에 주소 객체를 넣어줌. parent.parent: 지금 위치부터 상위 하나씩. = sesac_local~ / for 상위 폴더 내에 있는 다른 파일들 먹여주려고
sys.path.append(str(Path(__file__).parent.parent))      
from preprocessing import faster_preprocessing


if __name__ == '__main__':
    from preprocessing import faster_preprocessing as faster
    ...
    train_loader, valid_loader = faster.get_nuts_dataloader(image_dir, label_dir)

    model = load_model(num_classes = 15)

    train(model, train_loader, epochs =1, save_path='./faster_nuts.pth')
